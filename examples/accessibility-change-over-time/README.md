...
**Key limitations.** The 800 m Euclidean catchment overstates walking coverage (no street-network routing). ACS 2023 five-year estimates are now three years stale. The DiD panel uses a simulated upgrade timeline---coefficients are illustrative, not causally interpretable. Real MTA Capital Program upgrade dates (via FOIL) and an outcome variable (ridership, property values) are needed before causal claims can be made. Significant Moran's *I* values mean OLS standard errors are likely underestimated; spatial HAC corrections are recommended.
