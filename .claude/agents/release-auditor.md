...
3. **pyproject metadata.** `name`, `authors`, `requires-python`, `license`,
   ...
   `[all]`) present and consistent. `nyc-geo-toolkit` pinned `>=0.3,<0.5`
   (widened in v0.5.1 to match nyc311 v1.0.2); `factor-factory` + `jellycell`
   gated behind `; python_version >= '3.12'` env markers.
