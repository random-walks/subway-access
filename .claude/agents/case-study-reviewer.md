...
1. **Station counts.** 493 active stations, 157 ADA-accessible. 101
   press-release-sourced upgrade years, 56 hash-fallback. Do not silently change
   these without surfacing the discrepancy.
...
3. **Model coefficients.** OLS on gap score: R² = .202, _F_(3, 2313) = 108.83.
   Moran's _I_ for gap = .2271 (_z_ = 40.87). Do not let a refactor drift these
   values.
4. **Report pipeline intactness.**
   ...
   - The three supplementary reports exist: `correlation-analysis.md`,
     `model-specification.md`, `spatial-diagnostics.md`.
5. **Narrative preservation.** `CASESTUDY.md` Abstract, Introduction,
   ...
   edit.
6. **Engine-audit appendix additive.** New factor-factory engine fits land as a
...
7. **Factor-factory engines are optional.** The case study must still run
...
8. **Data provenance caveats.** The 56-station hash-fallback caveat is
