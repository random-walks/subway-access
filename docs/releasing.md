...
`docs/images/subway-access-hero.png`. PyPI renders the image via a **raw GitHub URL**
in `README.md` (pointing at `main`); after you change the PNG, merge to `main` so
the link matches. To refresh the graphic, run the `examples/about-the-data` example
and copy `reports/figures/map-library-header-horizontal.png` over
