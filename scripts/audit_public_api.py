"""Audit the explicit public subway_access namespace surface."""
...
DOC_DIRECTIVE_RE: Final[re.Pattern[str]] = re.compile(r"^:::\s+(subway_access)\s*$")
...
def _validate_docs_api_coverage() -> None:
    ...
    if documented != {"subway_access"}:
        raise ValueError(
            "docs/api.md public namespace coverage mismatch: expected only `subway_access`."
        )


def main() -> int:
    module_name = "subway_access"
    module = importlib.import_module(module_name)
    names = getattr(module, "__all__", None)
    if not isinstance(names, list):
        raise ValueError("subway_access must define __all__ as a list.")

    import_map = _build_import_map(module_name)
    rows: list[AuditRow] = []
    for symbol_name in names:
        if not isinstance(symbol_name, str):
            raise ValueError("subway_access.__all__ must contain only strings.")
        if symbol_name.startswith("_") and symbol_name not in ALLOWED_PRIVATE_EXPORTS:
            raise ValueError(
                f"subway_access.__all__ must not export private name {symbol_name!r}."
            )
        if not hasattr(module, symbol_name):
            raise ValueError(
                f"subway_access.__all__ references missing symbol {symbol_name!r}."
            )
